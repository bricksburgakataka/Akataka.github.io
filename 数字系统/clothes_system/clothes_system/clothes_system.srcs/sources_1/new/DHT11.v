...
//时钟为50MHZ，20ns.现：1000=MHZ
localparam	TIME18ms = 23'd1_800_099;    //开始态的拉低18ms,900_000个时钟周期，这里适当的延长了拉低时间。原：900_099
localparam	TIME35us = 16'd3_500;  //数据传输过程中，数据0拉高的出现。原：1_750
...
reg[39:0]	dht11_data;     //原：2，22，5，39
    ...
    begin
	   ...
	   S_START_DHT11:
		  if((DHT11_Cnt > TIME35us) && dht11_negedge == 1'b1)  //延时一段时间后，通过判断dht11总线的下降沿，是否结束响应
			 ...
			 next_state <= S_START_DHT11;
	   S_DATA:
		  if(DHT11Bit_Cnt == 'd39 && dht11_negedge == 1'b1)  //接收到40bit数据后，进入停止态.原：39
			 next_state <= S_STOP;
		  else
			 next_state <= S_DATA;
	   S_STOP:
...
always@(posedge sys_clk or negedge rst_n)
    begin
	   if(rst_n == 1'b0)
		  ...
		  if((DHT11_Cnt <= TIME35us + 16'd6_000) && dht11_negedge == 1'b1) //原：d3000为低电平时间，高电平持续时间低于35us认为是数据0
